def tsp(cities, paths, dist):
    """Checks if there exists a path through all cities with a total distance less than the specified target."""
    cases = permutations(cities)
    for case in cases:
        prev = case[0]
        dist_sum = 0
        for city in case[1:]:
            curr_dist = paths[prev][city]
            dist_sum += curr_dist
            prev = city
        if dist_sum < dist:
            return True
