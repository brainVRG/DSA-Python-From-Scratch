def subset_sum(nums, target):
    """Determines if any subset of the given list sums up to the target value using brute-force recursion."""
    return find_subset_sum(nums, target, len(nums) - 1)

def find_subset_sum(nums, target, index):
    """Helper function that recursively explores including or excluding the current element to reach the target."""
    if target == 0:
        return True
    if index < 0:
        return False
