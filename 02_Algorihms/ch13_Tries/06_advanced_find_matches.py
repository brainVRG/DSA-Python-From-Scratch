class Trie:
    def advanced_find_matches(self, document, variations):
        """Finds substring matches in the document, accounting for character obfuscation using a variations dictionary."""
        matches = set()
        for i in range(len(document)):
            level = self.root
            for j in range(i, len(document)):
                ch = document[j]
                search_ch = variations.get(ch, ch)

                if search_ch not in level:
                    break

                level = level[search_ch]
