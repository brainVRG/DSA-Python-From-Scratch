class LinkedList:
    def add_to_head(self, node):
        """Adds a node to the head of the list in O(1) time, updating the tail if initially empty."""
    ...
    def add_to_tail(self, node):
        """Adds a node to the tail of the list in O(1) time using the tail reference."""
        if self.head is None:
            self.head = node
            self.tail = node
            return
    ...
    def __init__(self):
        """Initializes an empty linked list with both head and tail pointers."""
        self.head = None
        self.tail = None
    ...
    def __iter__(self):
        """Yields each node in the linked list from head to tail."""
        node = self.head
