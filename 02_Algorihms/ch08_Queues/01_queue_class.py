class Queue:
    ...
    def __init__(self):
        """Initializes an empty list to store queue items."""
        self.items = []

    def push(self, item):
        """Adds an item to the tail (index 0) of the queue."""
        self.items.insert(0, item)

    def pop(self):
        """Removes and returns an item from the head (last index) of the queue."""
        if self.items:
            item = self.items.pop()
            return item
        else:
    ...
    def peek(self):
        """Returns the item at the head of the queue without removing it, or None if empty."""
